...
int rightShiftArith(int x, int n) {  // this is a LOGICAL right shift
    // calculates floor(x / 2^n)
    numToBits(x); // put the bits of x in an array
...
int and_op() { 
    REGISTERS[rd] = bitwiseAnd(REGISTERS[rs1], REGISTERS[rs2]); 
}

int or_op() {
    REGISTERS[rd] = bitwiseOr(REGISTERS[rs1], REGISTERS[rs2]);
}

int xor_op() {
    REGISTERS[rd] = bitwiseXor(REGISTERS[rs1], REGISTERS[rs2]);
}
...
int sra_op() {
    REGISTERS[rd] = rightShiftArith(REGISTERS[rs1], REGISTERS[rs2]);
}
...
int ori_op() {
    // rd = rs1 | imm
    REGISTERS[rd] = bitwiseOr(REGISTERS[rs1], imm);
}

int andi_op() { 
    // rd = rs1 & imm
    REGISTERS[rd] = bitwiseAnd(REGISTERS[rs1], imm);
}

int xori_op() {
    REGISTERS[rd] = bitwiseXor(REGISTERS[rs1], imm);
}

int slli() {
    int imm_bits = getnbits(4, 0, imm);
    REGISTERS[rd] = leftShift(REGISTERS[rs1], imm_bits);
}
int srli()   { return 0; }
int srai()   { return 0; }
int slti()   { return 0; }
...
int main() {
    ...
    std::cout << "x5: " << REGISTERS[5] << std::endl;
    std::cout << "x6: " << REGISTERS[6] << std::endl;
}
